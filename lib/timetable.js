...
Timetable.prototype.toICAL = function() {
  ...
  calendar.add(new ical.Timezone("Europe/Dublin"));

  calendar.setAttribute("X-WR-TIMEZONE", "Europe/Dublin");
  ...
  for(var i = 0, il = days.length; i < il; i++) {
    ...
    for(var j = 0, jl = periods.length; j < jl; j++) {
      ...
      event.setAttribute("CREATED", now.toICAL() + "Z");
      event.setAttribute("DTSTAMP", now.toICAL() + "Z");
      event.setAttribute("LAST-MODIFIED", now.toICAL() + "Z");
